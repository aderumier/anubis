...
// CI jobs that only want part of the matrix narrow it with these environment
// variables (see .github/workflows/debian-13-deb.yml). Each is a comma
// separated allow-list matched against the build method's own name
// ("debian", "rpm", "sysext", "tarball"), the GOOS, or the GOARCH. Leaving one
// unset means "no filter", so a plain `yeet` run still builds everything.
const allowList = (name) => {
  const raw = yeet.getenv(name);
  if (!raw) {
    return null;
  }
  return raw
...
const packages = methods
  ...
  .map(([goos, goarch, methods]) => {
    return methods
      .filter((method) => wanted(wantedMethods, method.name))
      .map((method) => {
...
// These source tarballs are expensive to produce (`go mod vendor`, `npm ci`),
// so a run narrowed to another method skips them along with every other
// tarball.
if (wanted(wantedMethods, "tarball")) {
